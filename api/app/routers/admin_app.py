...
def admin_refund_topup(
    ...
    txn = AccountBillingTransaction(user_id=original.user_id, initiated_by_user_id=owner.id, transaction_type="refund", product_type=original.product_type, product_code=original.product_code, description=f"Admin refund for transaction #{original.id}", status="succeeded", amount_minor=-abs(amount_refunded), currency=original.currency, quantity=reversal_quantity, parent_transaction_id=original.id, stripe_customer_id=original.stripe_customer_id, stripe_payment_intent_id=pi_id, stripe_invoice_id=payload.invoice_id, stripe_refund_id=str(getattr(refund, "id", "") or ""), stripe_credit_note_id=str(getattr(credit_note, "id", "") or "") if credit_note is not None else None, idempotency_key=f"stripe:refund:{str(getattr(refund, "id", "") or "")}", metadata={"source": "admin_refund_api"}, sms_ledger_processed_at=datetime.now(timezone.utc))
    db.add(txn)
    db.flush()
